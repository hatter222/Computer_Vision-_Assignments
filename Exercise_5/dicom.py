...
class ReadableDicomDataset():
    def __init__(self, filename):
        ...
        stack = all([dims_instances[x][0] == dims_instances[0][0] for x in range(len(dims_instances))]) and \
                all([dims_instances[x][1] == dims_instances[0][1] for x in range(len(dims_instances))]) and len(
            dims_instances) > 1
        ...
        try:
            ...
            self.geometry_imsize = [(self._dsstore[k][0x48, 0x6].value, self._dsstore[k][0x48, 0x7].value) for k in
                                    self.levels]
            self.geometry_tilesize = [(self._dsstore[k].Columns, self._dsstore[k].Rows) for k in self.levels]
            self.geometry_columns = [round(0.5 + (self.geometry_imsize[k][0] / self.geometry_tilesize[k][0])) for k in
                                     self.levels]
            self.geometry_rows = [round(0.5 + (self.geometry_imsize[k][1] / self.geometry_tilesize[k][1])) for k in
                                  self.levels]
            self.channels = self._ds[0x0028, 0x0002].value
            self.mpp_x = float(
                self._dsstore[0].SharedFunctionalGroupsSequence[0][0x028, 0x9110][0][0x028, 0x030][0]) * 1000
            self.mpp_y = float(
                self._dsstore[0].SharedFunctionalGroupsSequence[0][0x028, 0x9110][0][0x028, 0x030][1]) * 1000
    ...
    def level_downsamples(self):
        try:
            return [self._dsstore[0].TotalPixelMatrixColumns / self._dsstore[k].TotalPixelMatrixColumns for k in
                    self.levels]
        except:
            return [1]
    ...
    def level_dimensions(self):
        try:
            return [(self._dsstore[k].TotalPixelMatrixColumns, self._dsstore[k].TotalPixelMatrixRows) for k in
                    self.levels]
        except:
    ...
    def read_region_simple(self, location: tuple, level: int, size: tuple, zLevel: int = 0):
        ...
        if (self.numberOfFrames > 1) and not (self.multiInstanceStack):
            imgcut = self._dsstore[0].pixel_array[zLevel, location[1]:location[1] + size[1] - offset[0],
                     location[0]:location[0] + size[0] - offset[1]]
        elif (self.multiInstanceStack):
            imgcut = np.copy(self._dsstore[zLevel].pixel_array[location[1]:location[1] + size[1] - offset[0],
                             location[0]:location[0] + size[0] - offset[1]])
        else:
            imgcut = np.copy(self._dsstore[0].pixel_array[location[1]:location[1] + size[1] - offset[0],
                             location[0]:location[0] + size[0] - offset[1]])
    ...
    def get_id(self, pixelX: int, pixelY: int, level: int) -> (int, int, int):
        ...
        return (id_x, id_y), pixelX - (id_x * self.geometry_tilesize[level][0]), pixelY - (
                    id_y * self.geometry_tilesize[level][1]),
    ...
    def read_region(self, location: tuple, level: int, size: tuple, zLevel: int = None):
        ...
        # generate big image
        bigimg = 0 * np.ones(((rl[1] - lu[1] + 1) * self.geometry_tilesize[level][0],
                              (rl[0] - lu[0] + 1) * self.geometry_tilesize[level][1], 4), np.uint8)
        bigimg[:, :, 3] = 255
