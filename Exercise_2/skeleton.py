...
def vlad(files, mus, powernorm, gmp=False, gamma=1000):
    ...
    for f in tqdm(files):
        ...
        for k in range(mus.shape[0]):
            diff = desc - mus[k]
            f_enc = assignments[:, k].T.dot(diff)
        ...
        encodings = normalize(encodings, norm='l2', copy=False)
        # TODO
...
if __name__ == '__main__':
    ...
    # a) dictionary
   # print("train:", args.in_train, "suffix:", args.suffix, "labels:", args.labels_train)
    ...
    if not os.path.exists(fname) or args.overwrite:
        # TODO
        enc_test = vlad(files_test, mus,100)
